...
try:
    config = oci.config.from_file(os.path.dirname(os.path.realpath(sys.argv[0])) + r"\oci_config", "DEFAULT")
...
def error_log(message):
    error_log = open(os.path.dirname(os.path.realpath(sys.argv[0])) + r'\error.log', 'a+')
...
try:
    file = open(os.path.dirname(os.path.realpath(sys.argv[0])) + r'\settings.json', 'rb')
...
def first_contact():
    cluster_group = subprocess.Popen([r'%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe','Get-ClusterGroup'], shell=True, stdout=subprocess.PIPE)
